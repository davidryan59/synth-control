...
class SynthControlMessage {
  ...
  setTimeStart(tStart, tMult = 1) {
    if (Number.isFinite(tStart) && Number.isFinite(tMult) && tMult > 0) {
      this.c.t0 = tStart;
      this.c.t1 = tStart + tMult * this.duration;
    }
    return this;
  }

  get isTimeSet() { return Number.isFinite(this.c.t0); }

  get timeStart() { return this.isTimeSet ? this.c.t0 : 0; }

  get timeEnd() { return this.isTimeSet ? this.c.t1 : this.duration; }
  ...
  toString() {
    ...
    if (this.isTimeSet) {
      durationText = `time ${dn(this.timeStart)} to ${dn(this.timeEnd)} (duration ${dn(this.duration)})`;
    } else {
