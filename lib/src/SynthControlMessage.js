...
class SynthControlMessage {
  constructor(...paramArray) {
    ...
    // this.vE to be undefined (and this.isConstant is true), unless specified in construction data.
  ...
  get isConstant() { return !(Number.isFinite(this.vE) && this.v !== this.vE); }
  ...
  mapValue(data) {
    if (isObject(data)) {
      ...
      const { num } = data;
      if (mathOp && (unaryOps[dataOp] || Number.isFinite(num))) {
        if (this.isConstant) return new this.constructor(this, { v: mathOp(this.v, num) });
  ...
  // Swap start and end values, if both exist
  mapFlip(data) {
    if (isObject(data)) {
      if (isArray(data.params)) {
        if (!data.params.includes(this.param)) return this;
