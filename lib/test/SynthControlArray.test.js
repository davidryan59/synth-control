...
test('', () => {
  ...
  SynthControlMessage: param 'gain', length 3.6758676, value -0.768677
  SynthControlMessage: param '', length 0, value 0
  ...
...
test('Change a length for all parameters', () => {
  mapSca = tf.sca3.map({ type: 'length', op: '*', num: 7 });
  expect(mapSca.get(0).length).toEqual(4 * 7);
  expect(mapSca.get(1).length).toEqual(3 * 7);
  expect(mapSca.get(2).length).toEqual(2 * 7);
});
...
test('Reverse order of an SCA', () => {
  ...
  expect(mapSca.get(1).length).toEqual(3);
...
test('Change a length, using array (length 1) of maps', () => {
  mapSca = tf.sca3.map([
    {
      type: 'length', op: '*', num: 5, params: ['freq'],
    },
  ]);
  expect(mapSca.get(0).length).toEqual(4 * 5); // changed
  expect(mapSca.get(1).length).toEqual(3); // unchanged
});
...
test('Combine two maps using array', () => {
  mapSca = tf.sca3.map([
    ...
    {
      type: 'length', op: '*', num: 345, params: ['gain'],
    },
  ]);
  expect(mapSca.get(0).length).toEqual(4);
  expect(mapSca.get(0).value).toEqual(275);
  expect(mapSca.get(0).valueEnd).toBeUndefined();
  expect(mapSca.get(1).length).toEqual(3 * 345);
  expect(mapSca.get(1).value).toEqual(5);
  expect(mapSca.get(1).valueEnd).toEqual(7);
  expect(mapSca.get(2).length).toEqual(2);
