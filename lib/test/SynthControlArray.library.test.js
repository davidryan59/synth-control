...
const library = {
  ...
  label02: {
    level: 2,
    contents: [
      { param: 'treble', length: 4.689, value: -0.356 },
      { label: 'label01' },
      { label: 'label01', maps: [{ type: 'length', op: '*', num: 2 }] },
    ],
  },
  ...
  edge07: {
    level: 1,
    contents: [{ contents: 'contents item has neither label nor param' }],
  },
};
...
test('Construct level 2 (iterated) library entry', () => {
  ...
  expect(sca.get(3).length).toEqual(12);
  expect(sca.get(6).length).toEqual(24);
});
...
test('contents item has neither label nor param', () => {
