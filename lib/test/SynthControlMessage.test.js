...
// mapValue: valid cases
test('', () => { expect(tf.scm1.mapValue({ fn: (v) => 1 + 2 * v }).value).toEqual(881); });
test('', () => { expect(tf.scm3.mapValue({ fn: (v) => 10000 * v - 1 }).value).toBeCloseTo(344); });
test('', () => { expect(tf.scm3.mapValue({ fn: (v) => 10000 * v - 1 }).valueEnd).toBeCloseTo(677); });

// mapValue: invalid cases
test('', () => { expect(tf.scm1.mapValue().value).toEqual(440); });
test('', () => { expect(tf.scm1.mapValue({}).value).toEqual(440); });
test('', () => { expect(tf.scm1.mapValue({ fn: () => 'v' }).value).toEqual(440); });
test('', () => { expect(tf.scm3.mapValue({ fn: () => 'v' }).value).toEqual(0.0345); });
