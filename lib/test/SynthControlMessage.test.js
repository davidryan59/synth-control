...
beforeEach(() => {
  ...
  tf.scm6 = new SynthControlMessage(tf.scm1, { start: 550, end: 550 });
});
...
test('', () => { expect(tf.scm4.duration).toEqual(tf.scm1.duration); });
...
// Check a unary operation works correctly on duration
// (duration must be positive, but cover the case anyway)
test('', () => {
  expect(tf.scm2.map(
